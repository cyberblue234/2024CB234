...
Elevator::Elevator(Limelight *limelight3, Drivetrain *swerve)
{
...
double Elevator::CalculateSpeakerAngle(bool useLimelight)
{
    // Distance from speaker in meters
    double distance;
    if (useLimelight)
        distance = limelight3->GetDistanceFromTarget();
    else
        distance = swerve->GetDistanceFromSpeaker();
    if (distance < 1.45) return -52;
    if (distance < 1.6) return -51;
    if (distance < 1.8) return -49;
    if (distance < 2.0) return intakeAngle;
    // Height we are targetting in meters
    double targetHeight = ElevatorConstants::kSpeakerHeight + ((distance / ElevatorConstants::kForce) * ElevatorConstants::kGravity);
